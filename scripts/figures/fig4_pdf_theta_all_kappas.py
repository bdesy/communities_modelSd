...
parser.add_argument('-v', '--verif', type=bool, default=False,
                        help='show histograms of pdf from sample')
parser.add_argument('-l', '--limit', type=bool, default=False,
                        help='show beta to infty limit')
...
for D in range(Dmax, Dmin-1, -1):
    ...
    if verif:
        if dd=='pwl':
            dist = np.loadtxt('data/D{}-gamma{}-beta{}.txt'.format(D, 2.5, ratio))
        elif dd=='exp':
            dist = np.loadtxt('data/'+dd+'-D{}-lambda{}-beta{}.txt'.format(D, 10.0, ratio))
        if len(dist)>1e6:
            dist = dist[:1000000]
